...
describe("useBreweries", () => {
  ...
  describe("uniqueDepartmentsNumber calculation", () => {
    it("should calculate unique departments from breweries", () => {
      const breweriesWithDepartments = [
        { id: "1", name: "Brewery 1", postal_code: "75001" },
        { id: "2", name: "Brewery 2", postal_code: "75002" },
        { id: "3", name: "Brewery 3", postal_code: "13001" },
        { id: "4", name: "Brewery 4", postal_code: "75001" }, // Duplicate department
      ];

      mockUseBreweryStore.mockReturnValue({
        breweries: breweriesWithDepartments,
        loading: false,
        error: null,
        pagination: { page: 1, per_page: 20, total: 0 },
        selectedDepartment: "",
      ...
      expect(result.current.uniqueDepartmentsNumber).toEqual(["13", "75"]);
    });

    it("should handle empty postal codes", () => {
      const breweriesWithEmptyPostalCodes = [
        { id: "1", name: "Brewery 1", postal_code: "75001" },
        { id: "2", name: "Brewery 2", postal_code: "" },
        { id: "3", name: "Brewery 3", postal_code: "13001" },
        { id: "4", name: "Brewery 4", postal_code: "   " },
      ];

      mockUseBreweryStore.mockReturnValue({
        breweries: breweriesWithEmptyPostalCodes,
        loading: false,
        error: null,
        pagination: { page: 1, per_page: 20, total: 0 },
        selectedDepartment: "",
    ...
    it("should return empty array when no breweries", () => {
      mockUseBreweryStore.mockReturnValue({
        breweries: [] as Brewery[],
        loading: false,
        error: null,
      ...
      expect(result.current.uniqueDepartmentsNumber).toEqual([]);
    });
  });
});
